 ...
 *
 * PressureSensor samples the pressure sensor on the node with a given sample rate.
 * 
 * Created: 30.01.2008
 * Last modified: 30.01.2008
 */

public class PressureSensor extends Task {
    
    public final static String NAME   = "pressuresensor";
    ...
    public PressureSensor(PressureSensor ps) {
    	super(ps);
        m_iSamplerate       = ps.m_iSamplerate;
        m_iSamplesPerPacket = ps.m_iSamplesPerPacket;
    }
    ...
    public boolean setInputPortDatarate( int port, float pktPerSecond, int pktSize ) {
        System.err.println("Error: Task PressureSensor: received input port datarate! No input port exists here!");
        return false; 
    }
