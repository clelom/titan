...
//Changed_MR: File replaced by TitanJava
...
//Changed_MR: enum workaround for old compiler
	private class CFGState {
		final static int INIT =1;
		final static int TASKS=2;
		final static int CONNECTIONS=3;
    ...
    private ArrayList m_Connections = new ArrayList(); //Changed_MR: Removed Generic
    ...
    private void loadTask(Task newTask) {
        m_TaskMap.put(newTask.getName().toLowerCase(), newTask.clone());
        m_TaskMap.put(""+newTask.getID(), newTask.clone());
    }
    ...
    {
        //TODO: Read here the tasks from the JAR file!
        loadTask(new Accelerometer());
        loadTask(new AliasTask());
        loadTask(new Communicator());
        loadTask(new Covariance());
        loadTask(new DecisionTree());
        loadTask(new Duplicator());
        loadTask(new Fuzzy());
        loadTask(new KNN());
        loadTask(new Led());
        loadTask(new LightSensor());
        loadTask(new Magnitude());
        loadTask(new Mean());
        loadTask(new Min());
        loadTask(new Max());
        loadTask(new PressureSensor());
        loadTask(new SimilaritySearch());
        loadTask(new SimpleWriter());
        loadTask(new Sink());
        loadTask(new Sum());
        loadTask(new Switch());
        loadTask(new Threshold());
        loadTask(new TransDetect());
        loadTask(new Variance());
        loadTask(new Voice());
        loadTask(new ZeroCross());

        loadTask(new GSense());
        loadTask(new GraphPlot());
        loadTask(new RoundTrip());

        loadTask( new TitanFileWriter() );
        loadTask( new BTSensor() );
        loadTask( new VirtualBTSensor() );
        loadTask( new BTStatusGUI() );
        loadTask( new LabelingGUI() );
        loadTask( new SyncMerger() );
        loadTask( new DemoGUI() );
        loadTask( new Splitter() );
        loadTask( new StreamSelector() );
        loadTask( new StdDev() );
        /*
        try {
    ...
    private String processConnections(String strLine) throws CfgError {
        ...
        strConnection= replaceAll(strConnection," ", ""); //Changed_MR= new replaceAll methode
        strConnection=replaceAll(strConnection, "\t", "");
        ...
        String [] strInfo = split(strConnection,",");
        if (strInfo.length !=4 ){
    ...
    private void getTask( String strTask, String strArguments, String strAttributes ) throws CfgError {
        ...
        String[] args = null;
        HashMap attributes = null; //Changed_MR
        ...
        if ( strArguments != null ) {
            ...
            if (st.hasMoreTokens())
            {
                ArrayList arguments = new ArrayList(); //Changed_MR
        ...
        if (strAttributes != null) {
            ...
            attributes = new HashMap();//Changed_MR
    ...
    private void processTask(String strTask, String[] strArguments, Map attributes) throws CfgError {//Changed_MR
        ...
        if (attributes != null)
        {
            for (Iterator i = attributes.keySet().iterator(); i.hasNext(); ) {//Changed_MR
                String strKey = (String)i.next();
                newTask.addAttribute(strKey, (String)attributes.get(strKey)); //Changed_MR
            }
        }
...
//      System.out.println( "Added task: " + strTask );
    ...
    public boolean getTasks(String pckgname) throws ClassNotFoundException {
        ...
        try {
            ...
            strFile =replaceAll(strFile,"%20"," ");//Changed_MR
