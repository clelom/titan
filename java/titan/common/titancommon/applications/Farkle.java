...
import titancommon.applications.FarkleScore.PickUpException;
...
public class Farkle implements Application {

	public static final int[] ORIG_DICE = { 2, 3, 4, 5, 6, 7 };
	public static final int DICE = ORIG_DICE.length;
	public static final int PLAYERS = 2;

	private static final int PICKUP_LIMIT = 3;
	private static final int SHAKE_LIMIT = 5;
	private static final int ROLL_LIMIT = 5;
	private static final int SCORE_LIMIT = 2;
	
	// Important Titan objects
	FarkleScore m_farkleScore = new FarkleScore();
	TitanCommunicate m_com;
	ServiceDirectory m_sd;
	MessageDispatcher m_md; // TaskNetwork templates read from configuration
							// files
	TaskNetwork m_pickup;
	TaskNetwork m_shake;
	TaskNetwork m_roll;
	TaskNetwork m_score; // GUI
	FarkleFrame m_frame = new FarkleFrame(); // / task network adapted to the
												// current network
	...
	int [] m_playerScores = new int[PLAYERS];
	int m_curPlayer;
	...
	public boolean startApplication(TitanCommunicate com, ServiceDirectory sd,
		...
		m_frame.setDice(4, 0, FarkleFrame.DICE_NEUTRAL);
		m_frame.setDice(5, 0, FarkleFrame.DICE_NEUTRAL);
		m_frame.show();
		...
		System.out.println("Starting Farkle game");
	...
	public void update(Observable source, Object obs) {
		...
		if (obs instanceof DataMsg) {
			...
			m_playerScores[m_curPlayer] += m_farkleScore.getScore();
	...
	private void handleStateInit(DataMsg msg) {
		...
		// report state
		m_frame.setMessage("Got eyes from " + iDone + "/"+ m_diceValues.length + " dice (" + msg.destPort + "/" + msg.data[0] + ")");

		// if all dice have responded, start game - go to PICKUP state
		if (iDone == m_diceValues.length) {
			m_state = FSM_PICKUP;
			m_eventCounter = new int[DICE];
			m_farkleScore = new FarkleScore();
	...
	private void handleStatePickup(DataMsg msg) {
		...
		if (msg.data[0] == 1) {
			...
			boolean bAbleToContinue = false;
			try {
				bAbleToContinue = m_farkleScore.pickUp(msg.destPort);
			} catch (PickUpException e) {
				System.out.println(e);
				//m_frame.setMessage(e.toString());
				m_eventCounter[msg.destPort] = 0; // put back event counter
				return;
			}
			...
			if (maxevent > PICKUP_LIMIT && bAbleToContinue) {
				
				// check whether we can move on
				boolean [] blocked;
				try {
					blocked = m_farkleScore.startShake();
				} catch (PickUpException e) {
					System.err.println(e);
					m_frame.setMessage(e.toString());
					for(int i=0; i<m_eventCounter.length;i++) { 
						m_eventCounter[i] = 0;
						m_frame.setDice(i, m_diceValues[i],FarkleFrame.DICE_NEUTRAL);
						m_farkleScore.placedDown(i);
					}
					return;
				}
				...
				for (int i = 0; i < m_eventCounter.length; i++) {
					if (m_eventCounter[i] == 0 || blocked[i]) {
						m_frame.setDice(i, m_diceValues[i],FarkleFrame.DICE_BLOCKED);
				...
				if (loadNextTaskNetwork(m_shake)) {
					...
					m_frame.setMessage("Please shake the dice");
				} else {
					System.err
							.println("Could not advance from pickup to shake!");
				}
			} else if (maxevent > PICKUP_LIMIT) { // we're not ready to move on - what's the problem?
				boolean [] blocked;
				try {
					blocked = m_farkleScore.startShake();
				} catch (PickUpException e) {
		...
		if (msg.data[0] == 1) {
			...
			if (bAllShaken) {
				...
				if (loadNextTaskNetwork(m_roll)) {
					...
					m_frame.setMessage("Please throw the dice");
				} else {
					System.err
		...
		if (msg.data[0] == 0) {
			...
			if (bAllThrown) {
				...
				if (loadNextTaskNetwork(m_score)) {
					m_state = FSM_SCORE;
					System.out.println("Switched to state Rolling");
					m_frame.setMessage("Determining score...");
				} else {
					System.err
	...
	private void handleStateScore(DataMsg msg) {
		...
		if (allScored) {
			...
			int score;
			try {
				score = m_farkleScore.throwDone(m_diceValues);
				m_frame.setScore(m_curPlayer, score, m_playerScores[m_curPlayer]);
				if (score == 0) {
					m_frame.setMessage("!!! FARKLE !!!");
					m_farkleScore = new FarkleScore();
					m_curPlayer = (m_curPlayer+1)%PLAYERS;
				}
				
			} catch (PickUpException ex) {
				m_frame.setMessage("Scoring failed!");
				ex.printStackTrace();
			}
