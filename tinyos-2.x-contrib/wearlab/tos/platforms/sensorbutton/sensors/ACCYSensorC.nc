...
/**
 * ADC ACC CHANNEL Y for Sensor Button ADC
 *
 * - Revision -------------------------------------------------------------
 * $Revision: 1.1 $
 * $Date: 2006/10/31
...
generic configuration ACCYSensorC()
{
  provides {
...
implementation
{
  ...
  Read = AdcReadClient;
  AdcReadClient.Msp430Adc12Config -> Settings.Msp430Adc12Config[ACC_CHANNEL_Y];
  
  components new AdcReadNowClientC() as AdcReadNowClient;
  ReadNow = AdcReadNowClient;
  ReadNowResource = AdcReadNowClient;
  AdcReadNowClient.Msp430Adc12Config -> Settings.Msp430Adc12Config[ACC_CHANNEL_Y];
